...
function getValueOfUnit(unitInput) {
  ...
  if (typeof unitValue !== 'string') {
    throw new Error(`[sofjs-unit] the unit provided ${unitInput} doesn't exists, please use the one of the following units ${JSON.stringify(unitMap, null, 2)}`);
  }
...
function toWei(sophyInput, unit) {
  var sophy = numberToString(sophyInput); // eslint-disable-line
  const base = getValueOfUnit(unit);
  const baseLength = unitMap[unit].length - 1 || 1;

  // Is it negative?
  var negative = (sophy.substring(0, 1) === '-'); // eslint-disable-line
  if (negative) {
    sophy = sophy.substring(1);
  }

  if (sophy === '.') { throw new Error(`[sofjs-unit] while converting number ${sophyInput} to wei, invalid value`); }

  // Split it into a whole and fractional part
  var comps = sophy.split('.'); // eslint-disable-line
  if (comps.length > 2) { throw new Error(`[sofjs-unit] while converting number ${sophyInput} to wei,  too many decimal points`); }

  var whole = comps[0], fraction = comps[1]; // eslint-disable-line

  if (!whole) { whole = '0'; }
  if (!fraction) { fraction = '0'; }
  if (fraction.length > baseLength) { throw new Error(`[sofjs-unit] while converting number ${sophyInput} to wei, too many decimal places`); }
